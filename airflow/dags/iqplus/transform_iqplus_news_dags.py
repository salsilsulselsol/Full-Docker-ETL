...
def transform_and_load_news(**kwargs):
    """
    Fungsi utama untuk transformasi dan load dengan solusi final untuk IndexError.
    """
    dag_run = kwargs.get('dag_run')
    client = get_mongo_client()
    
    # --- PERUBAHAN: Ambil tokenizer dan summarizer dari dictionary ---
    models = load_models()
    ...
    for i, article in enumerate(pending_articles):
        ...
        try:
            ...
            # #######################################################################
            # ### SOLUSI FINAL DAN PALING PASTI UNTUK INDEXERROR ###
            # #######################################################################
            # 1. Gunakan tokenizer untuk memotong teks berdasarkan panjang token maksimal model.
            #    Ini adalah cara yang dijamin berhasil, bukan lagi perkiraan.
            #    tokenizer.model_max_length untuk BART adalah 1024.
            ...
            # 2. Decode kembali token yang sudah pasti aman menjadi string.
            safe_content = tokenizer.decode(
                token_ids,
                skip_special_tokens=True # Menghapus token spesial seperti <s>
            )
            # #######################################################################
            
            # 3. Gunakan 'safe_content' yang sudah dijamin pas untuk diringkas.
            summary_result = summarizer(
                safe_content, 
                max_length=150, 
                min_length=50, 
